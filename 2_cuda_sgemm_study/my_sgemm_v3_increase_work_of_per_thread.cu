...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K)
{
    ...
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    float *A_ptr_start = A_ptr + STEP * blockIdx.x * K;
    float *B_ptr_start = B_ptr + STEP * blockIdx.y;
    ...
    for (int s = 0; s < K; s += STEP)
    {
        for(int i = 0; i < STRIDE; i++){
            for(int j = 0; j < STRIDE; j++){
                a_shared[tx + i * BLOCK_SIZE][ty + j * BLOCK_SIZE] = A_ptr_start[(tx + i * BLOCK_SIZE) * K + s + ty + j * BLOCK_SIZE];
                b_shared[tx + i * BLOCK_SIZE][ty + j * BLOCK_SIZE] = B_ptr_start[(tx + i * BLOCK_SIZE + s) * N + ty + j * BLOCK_SIZE];
            }
        }
        __syncthreads();
        for(int i = 0; i < STRIDE; i++){
            for(int j = 0; j < STRIDE; j++){
                ...
                {
                    temp[i][j] += a_shared[tx + i * BLOCK_SIZE][k] * b_shared[k][ty + j * BLOCK_SIZE];
                }
            }
        }
        __syncthreads();
    }
    float * C_ptr_start = C_ptr + (blockIdx.x * STEP) * N + blockIdx.y * STEP;
    for(int i = 0; i < STRIDE; i++){
        for(int j = 0; j < STRIDE; j++){
            C_ptr_start[(tx + i * BLOCK_SIZE) * N + ty + j * BLOCK_SIZE] = temp[i][j];
        }
    }
}
