...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K)
{
    ...
    float *A_ptr_start = A_ptr + blockDim.x * blockIdx.x * K;
    float *B_ptr_start = B_ptr + blockDim.y * blockIdx.y;
    ...
    {
        a_shared[threadIdx.x][threadIdx.y + s] = A_ptr_start[threadIdx.y + s + threadIdx.x * K];
        b_shared[threadIdx.x + s][threadIdx.y] = B_ptr_start[(threadIdx.x + s) * N + threadIdx.y];
    }
