...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K)
{
    ...
    for (int s = 0; s < K; s += NUM_PRE_BLOCK_K_)
    {
        ...
        for(int k = 0; k < NUM_PRE_BLOCK_K_; k++){
            ...
            for(int j = 0; j < NUM_PRE_THREAD_N_; j++){
                b_reg[j] = b_shared[k][ty * NUM_PRE_THREAD_N_ + j];
            }
            for(int i = 0; i < NUM_PRE_THREAD_M_; i++){
                for(int j = 0; j < NUM_PRE_THREAD_N_; j++){
    ...
    {
        for(int j = 0; j < NUM_PRE_THREAD_N_; j++){
            C_ptr_start[N * (i + tx * NUM_PRE_THREAD_M_) + ty * NUM_PRE_THREAD_N_ + j] = tmp[i][j];
        }
    }
}
...
int main()
{
    ...
    const int NUM_PRE_THREAD_M = 2;
    const int NUM_PRE_THREAD_N = 2;
    dim3 block(NUM_PRE_BLOCK_M / NUM_PRE_THREAD_M, NUM_PRE_BLOCK_K / NUM_PRE_THREAD_N);
